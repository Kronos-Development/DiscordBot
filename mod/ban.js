...
module.exports = {
  ...
  run: async (bot, message, args) => {
    const role = Utils.findRole(config.Permissions.Staff_Commands.Ban, message.guild);
    const user = Utils.ResolveUser(message);
    const reason = args.slice(1).join(" ");

    if (!role) return message.channel.send(Embed({ preset: 'console' }));
    const logs = Utils.findChannel(config.Logs.Punishments.Channel, message.guild, type = 'text');
    if (config.Logs.Punishments.Enabled == true && !logs) return message.channel.send(Embed({ preset: 'console' }));
    if (!Utils.hasPermission(message.member, config.Permissions.Staff_Commands.Ban)) return message.channel.send(Embed({ preset: 'nopermission' }));
    ...
    if (config.Punishment_System.Punish_Staff === true) {
      ...
      if (Utils.hasPermission(user, config.Permissions.Staff_Commands.Ban)) return message.channel.send(Embed({ preset: 'error', description: lang.ModerationModule.Errors.CantPunishStaff }))
    }
    ...
    message.channel.send(Embed({ title: lang.ModerationModule.Commands.Ban.Title, description: lang.ModerationModule.Commands.Ban.Description.replace(/{user}/g, user).replace(/{userid}/g, user.id), color: config.Success_Color }));
  },
  description: lang.Help.CommandDescriptions.Ban,
