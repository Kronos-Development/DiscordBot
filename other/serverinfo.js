...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        let channels = message.guild.channels.cache.filter(c => c.type == 'text').map(c => ' <#' + c.id + '>').toString();
        ...
        message.channel.send(Embed({
            ...
            fields: [
                ...
                { name: lang.Other.OtherCommands.Serverinfo.Fields[2].Name, value: lang.Other.OtherCommands.Serverinfo.Fields[2].Value.replace(/{humans}/g, message.guild.members.cache.filter(m => m.user.bot == false).size).replace(/{bots}/g, message.guild.members.cache.filter(m => m.user.bot == true).size).replace(/{total}/g, message.guild.memberCount) },
                { name: lang.Other.OtherCommands.Serverinfo.Fields[3], value: region },
                { name: lang.Other.OtherCommands.Serverinfo.Fields[4].replace(/{amt}/g, message.guild.channels.cache.size), value: channels },
